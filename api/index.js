...
// -------------------
// VULNERABLE: SQL Injection Demonstration Endpoint
// -------------------
// NONCOMPLIANT CODE - FOR DEMONSTRATION PURPOSES ONLY
app.get("/api/vulnerable-user-search", (req, res) => {
  ...
  let query = "SELECT id, username, email, name, created_at FROM users WHERE ";
  
  if (username && email) {
    // VULNERABLE: Both parameters can be injected
    query += `username = '${username}' AND email = '${email}'`;
  } else if (username) {
    // VULNERABLE: Username parameter injection
    query += `username = '${username}'`;
  } else {
    // VULNERABLE: Email parameter injection  
    query += `email = '${email}'`;
  }
  
  console.log("Executing query:", query); // This exposes the query for demonstration
  ...
  db.query(query, (err, data) => {
    if (err) {
      ...
      return res.status(500).json({ 
        ...
        details: err.message, // Exposes internal database errors
        query: query // Exposes the vulnerable query
      });
    }
    ...
    res.status(200).json({
      success: true,
      users: data,
      query: query, // Exposes the executed query
...
// -------------------
// SECURE: SQL Injection Protection Demonstration
// -------------------
app.get("/api/secure-user-search", (req, res) => {
  const { username, email } = req.query;
  ...
  const validateInput = (input, type) => {
    ...
    switch (type) {
      case 'username':
        return /^[a-zA-Z0-9_-]{1,50}$/.test(input.trim());
      case 'email':
        return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(input.trim()) && input.length <= 100;
      default:
        return false;
    }
  };
  ...
  db.query(query, params, (err, data) => {
    if (err) {
      logEvent(`Database error in secure user search: ${err.message}`);
      // SECURE: Generic error message without sensitive details
      return res.status(500).json({ 
        error: "Search operation failed. Please try again." 
      });
    }
    
    // SECURE: Filtered response without exposing query details
    res.status(200).json({
      success: true,
      users: data.map(user => ({
        id: user.id,
